...
#import "LMAppDelegate+EaseMob.h"

/**
 *  本类中做了EaseMob初始化和推送等操作
 */

@implementation LMAppDelegate (EaseMob)
- (void)easemobApplication:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    NSString *appkey = @"mailing18#sandbox";
...
#warning SDK注册 APNS文件的名字, 需要与后台上传证书时的名字一一对应
    ...
    apnsCertName = @"laomazj_apns";
    if (![specifyServer boolValue])
    {
        EMError *eme=       [[EaseMob sharedInstance] registerSDKWithAppKey:appkey
                                           apnsCertName:apnsCertName
                                            otherConfig:@{kSDKConfigEnableConsoleLogger:[NSNumber numberWithBool:YES]}];
        NSLog([eme description]);
    }
    else
    {
